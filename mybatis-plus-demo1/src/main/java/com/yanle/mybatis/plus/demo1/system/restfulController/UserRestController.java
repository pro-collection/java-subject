...
import com.yanle.mybatis.plus.demo1.system.vo.UserVO;
...
public class UserRestController {
    ...
    public ApiResponse updateRole(@RequestBody UserVO userVO){
        ...
        SysUser sysUser = new SysUser(
                ...
                userVO.getBirthday(),
                userVO.getHobby(),
                userVO.getLiveAddress(),
                null,
                new Date()
        );
        try {
            sysUserRoleService.deleteByUserId(userVO.getId());
            sysUserRoleService.insert(
                    new SysUserRole(userVO.getId(),sysRoleService.getIdByName(userVO.getUserRole()))
            );
